...
def run_and_display(
    ...
    if run_baseline:
        ...
        prev_config = inf_config["save_prefix"]
        inf_config["save_prefix"] = os.sep.join([inf_config["save_prefix"], "without-ptp"])
        ...
        inf_config["save_prefix"] = prev_config
    ...
    save_images(images, f'{inf_config["save_prefix"]}generated_images.png') # unaffected by verbose, unlike every other save_images
...
def text2stereoimage_ldm_stable(
    ...
    if verbose:
        save_images(_latents_init, f'{inf_config["save_prefix"]}initial_latents.png')
    ...
    for i, t in enumerate(tqdm(model.scheduler.timesteps[-inf_config["num_ddim_steps"]:])):
        ...
        if (i % latents_editing_freq == 0) and verbose:
            ...
            save_images(_latents_at_t, f'{inf_config["save_prefix"]}latents_at_t={t}.png')
        ...
        if i == latents_editing_freq:
            ...
            if verbose:
                _latents_at_t = ptp_utils.latent2image(model.vae, latents)
                save_images(_latents_at_t, f'{inf_config["save_prefix"]}latents-after-shift_at_t={t}.png')
            ...
            if verbose:
                ...
                save_images(_mask, f'{inf_config["save_prefix"]}denoising-mask.png')
        ...
        if  (i > latents_editing_freq and i % latents_editing_freq == 0):
            ...
            if verbose:
                ...
                save_images(_latents_masked, f'{inf_config["save_prefix"]}latents-with-applied-mask_at_t={t}.png')
...
def estimate_disparity_from_gt(
    ...
    if verbose:
        ...
        save_hist_from_array(maps[0], f'{maps_folder}_DPT-depth_hist.png', title=r"Histogram of $Z_{DPT}$", color_idx=1)
...
def run_inv_sd(
    ...
    create_save_path_from_prefix(inf_config["output_prefix"])
...
if __name__ == "__main__":
    ...
    # maybe replace this? - issue #13 https://github.com/KarylReyne/single-view-stereo/issues/13#issue-3831974881
