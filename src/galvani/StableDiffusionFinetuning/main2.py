...
def forward_diffusion_process(x, t, noise_schedule):
    ...
    normal_noise = torch.randn_like(x)  # noise ~ N(0, I)
    return torch.sqrt(1-beta_t) * x + torch.sqrt(beta_t) * normal_noise


def reverse_diffusion_step(x, t, noise_schedule):
    beta_t = noise_schedule[t]
    normal_noise = torch.randn_like(x)  # noise ~ N(0, I)
    return (x - torch.sqrt(beta_t) * normal_noise) / torch.sqrt(1-beta_t)
...
def main(training_config, dataset_config):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, _) in enumerate(dataloader):
            images = images.to(training_config["device"])

            # Forward diffusion: Add noise
            t = torch.randint(0, timesteps, (images.size(0),)).to(training_config["device"])  # Random timestep for each image
            noisy_images = forward_diffusion_process(images, t, beta_schedule)
