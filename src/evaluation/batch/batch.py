...
"""
class BatchHandling:
  def __init__(self, gt_root_folder, gen_root_folder, left_name_suffix = "left.png", right_name_suffix = "right.png", gen_name_suffix = "gen.png"):
  ...
  def evaluate_batch(self):
    ...
    for folder_name in tqdm(folder_list, desc="Evaluating stereo pairs", unit="scene"):
      ...
      if not (os.path.exists(gt_left_path) and os.path.exists(gt_right_path) and os.path.exists(gen_image_path)):
        print(f"Skipping {folder_name}: Missing required images")
        continue 

      evaluator = StereoEvaluator(gt_left_path, gt_right_path, gen_image_path)
      evaluator.evaluate()
...
if __name__ == "__main__":
  evaluator = BatchHandling(
      gt_root_folder="/training_data",
      gen_root_folder="/generation",
  )
